...
namespace CsTech.Modularity
{
    ...
    /// are also used to prevent common deployment problems such as having a module that's required at startup that depends on modules
    /// that will only be downloaded on demand. 
    /// 
    ...
    public class ModuleInfoGroup : IModuleCatalogItem, IList<ModuleInfo>, IList // IList must be supported in Silverlight 2 to be able to add items from XAML
    {
        ...
        bool IList.Contains(object value)
        {
            ...
            if (moduleInfo == null)
            {
                throw new ArgumentException(ExceptionMessages.ValueMustBeOfTypeModuleInfo, nameof(value));
            }
        ...
        {
            return _modules.IndexOf((ModuleInfo)value);
        }
